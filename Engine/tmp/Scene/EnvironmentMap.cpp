...
void EnvironmentMap::loadHDRi(std::string filename) {
    ...
    if (data) {
        ...
        LOG_INFO("Failed to load HDRi [" + filename + "]");
    }
}
...
void EnvironmentMap::loadSkybox(std::string filename, std::string filetype) {
    ...
    for (GLuint i = 0; i < 6; i++) {
        ...
        if (data) {
            ...
            LOG_INFO("Loaded texture: [" + std::to_string(width) +
                "x" + std::to_string(height) + "], " + std::to_string(nrChannels) + " channels");
            stbi_image_free(data);
            SkyboxRes = width;
        }
        else {
            LOG_INFO("Cubemap texture failed to load at path: " + faces[i]);
            stbi_image_free(data);
        }
    }
...
void EnvironmentMap::preCompute() {
    ...
    LOG_INFO("   " + std::to_string(std::chrono::duration_cast<std::chrono::milliseconds>(duration).count()) + " Milliseconds to precompute environment");
}
