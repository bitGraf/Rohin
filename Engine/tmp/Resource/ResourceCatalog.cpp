...
void ResourceCatalog::updateResourceFromFile(void* data, u32 size) {
    ...
    if (!getResource(filename, ent)) {
        LOG_ERROR("Failed to find resource %s", filename);
    }
    else {
        switch (ent.type) {
        case resMesh:
            LOG_INFO("Updating Mesh Resource from %s", ent.filename.c_str());
            break;
        case resImage:
            LOG_INFO("Updating Image Resource from %s", ent.filename.c_str());
            break;
        case resFont:
            LOG_INFO("Updating Font Resource from %s", ent.filename.c_str());
            break;
        case resShader:
            LOG_INFO("Updating Shader Resource from %s", ent.filename.c_str());
            break;
        case resLevel:
            LOG_INFO("Reloading Level resource from %s", ent.filename.c_str());
            break;
        case resOther:
            LOG_INFO("Updating Other Resource from %s", ent.filename.c_str());
            break;
        }
    }
}
...
bool ResourceCatalog::loadResourceFile(std::string filename) {
    LOG_INFO("Reading resource file [%s]...", filename.c_str());
    size_t bytesRead;
    char* buffer = FileSystem::GetInstance()->readAllBytes("resources\\" + filename, bytesRead);

    if (buffer && bytesRead > 0) {
        LOG_INFO(" Succesfully read %zu kilobytes", bytesRead/1024);
    }
    else {
        LOG_ERROR("Failed to load resource file '%s'", filename.c_str());
        return false;
    }
    ...
    if (strncmp(buffer, correctSig, 10) == 0) {
        ...
        for (int n = 0; n < numMeshes; n++) {
            if (strncmp(data, "Mesh: ", 6) != 0) {
                LOG_ERROR("Error reading .mcf file. Expected a mesh entry and didnt get one.");
                break;
            }
        ...
        for (int n = 0; n < numMats; n++) {
            if (strncmp(data, "Material: ", 10) != 0) {
                LOG_ERROR("Error reading .mcf file. Expected a material entry and didnt get one.");
                break;
            }
        ...
        LOG_ERROR(" Incorrect file signature found...");
        return false;
    }
