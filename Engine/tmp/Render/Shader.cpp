...
void Shader::create(
    ...
    LOG_INFO("Building shader(" + ShaderName + "): [" + vShaderPath + "]|[" + fShaderPath + "]");

    /* load shader files */
    errno = 0;
    FILE *f = fopen((ShaderResourcePath + vShaderPath).c_str(), "rb");
    if (f == NULL) {
        LOG_INFO("   Failed to open file: " + vShaderPath);
        LOG_INFO("   Error: " + std::to_string(errno));
        LOG_INFO("   Path: " + (ShaderResourcePath + vShaderPath));
        return;
    }
    fseek(f, 0, SEEK_END);
    long vSize = ftell(f);
    ...
    if (f == NULL) {
        LOG_INFO("   Failed to open file: " + fShaderPath);
        return;
    }
    fseek(f, 0, SEEK_END);
    long fSize = ftell(f);
    ...
    if (!success) {
        ...
        LOG_INFO("  ERROR::SHADER::VERTEX::COMPILATION_FAILED(" + vShaderPath + ")\n" + std::string(infoLog));
    }
    //Fragment shader
    GLuint fShader;
    ...
    if (!success) {
        ...
        LOG_INFO("  ERROR::SHADER::FRAGMENT::COMPILATION_FAILED(" + fShaderPath + ")\n" + std::string(infoLog));
    }
    //Shader program
    ...
    if (!success) {
        ...
        LOG_INFO("  ERROR::SHADER::LINK_FAILED\n" + std::string(infoLog));
        //system("pause");
    }
    ...
    LOG_INFO("   " + std::to_string(std::chrono::duration_cast<std::chrono::microseconds>(duration).count()) + " Microseconds to compile");
}
